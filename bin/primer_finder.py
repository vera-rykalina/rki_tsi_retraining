...
def parse_primers(primer_fasta):
    ...
    for record in SeqIO.parse(primer_fasta, "fasta"):
        ...
        if header.lower().endswith("_s"):
            pair_name = header[:-2]
            primers.setdefault(pair_name, [None, None])[0] = seq
        elif header.lower().endswith("_as"):
            pair_name = header[:-3]
    ...
    # Keep only pairs where both primers present
    primers = {k: v for k, v in primers.items() if v[0] and v[1]}
    return primers

def find_approximate_match(primer_seq, ref_seq, max_mismatches=1):
    ...
    for i in range(len(ref_seq) - primer_len + 1):
        ...
        if mismatches <= max_mismatches:
            return i + 1, i + primer_len  # 1-based coords
    return None

# Updated function with added columns (primer lengths & product coords incl primers)
def find_primer_positions_approx(ref_genome, primer_pairs, max_mismatches=1):
    ...
    for pair_name, (sense_primer, antisense_primer) in primer_pairs.items():
        ...
        fwd_match = find_approximate_match(sense_primer, ref_seq, max_mismatches)
        if not fwd_match:
            print(f"Sense primer for pair '{pair_name}' not found (within {max_mismatches} mismatch).", file=sys.stderr)
            continue

        rev_match = find_approximate_match(antisense_primer_rc, ref_seq, max_mismatches)
        if not rev_match:
            print(f"Antisense primer for pair '{pair_name}' not found (within {max_mismatches} mismatch).", file=sys.stderr)
            continue

        fwd_start, fwd_end = fwd_match
        rev_start, rev_end = rev_match
