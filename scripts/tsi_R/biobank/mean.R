...
names(wes_palettes)
getwd()
din_table <- read_excel("/Users/vera/Desktop/Biobank/2020_08_07_biobank_din.xlsx",sheet = "din_labs_r")
head(din_table) 
nrow(din_table)
...
din_scatter1 <- din_table %>%
  ...
  theme_minimal() +
  theme(text=element_text(size=12, color="grey15"), panel.background = element_rect(fill = "white", colour = "grey80"), axis.title=element_text(size=12, color="grey15"),legend.text=element_text(size=11), legend.background =element_rect(color="white"))+
  theme(legend.position=c(0.77, 0.23), legend.key=element_rect(color="grey15"))
din_scatter1
...
raw_table <- read_excel("Desktop/2020_08_07_biobank_din.xlsx",sheet = "raw_data")
head(raw_table) 
tail(raw_table)
colnames(raw_table)
nrow(raw_table)
...
din_less_1 <- din_table %>%  #71  have DIN diff >0.6
...
din_less_than_1 <- din_table %>%  # 87 have DIN diff <= 1
  filter(abs(Difference) <= 1  & abs(Difference) > 0.5) %>%
  count()
...
crb.boxplot.size <- raw_table %>%
  ...
  geom_hline(yintercept = 7, size=0.8, color="#3A5894")+
  stat_boxplot(geom ='errorbar', aes(color=factor(code_CRB, levels = c("CRB1","CRB2","CRB3","CRB4","CRB5","CRB6","CRB7","CRB8","CRB9","CRB10","CRB11","CRB12","CRB13"))), lwd=0.7) + #lwd=1
  geom_boxplot(aes(color=factor(code_CRB, levels = c("CRB1","CRB2","CRB3","CRB4","CRB5","CRB6","CRB7","CRB8","CRB9","CRB10","CRB11","CRB12","CRB13"))), outlier.shape = 21)+ #lwd=0.8, fatten=0.9
  labs(x='Anonymized CRB ID', y='DIN')+
  scale_y_continuous(limits = c(1, 10), breaks = seq(1,10,1))+
  scale_color_manual(label=c(CRB1="444",CRB2="237",CRB3="177",CRB4="70",CRB5="80",CRB6="84",CRB7="154",CRB8="266",CRB9="213",CRB10="99",CRB11="147",CRB12="236",CRB13="429"), name ="Number of samples", values = c("#0099CC", "#8E8E38", "#53868B","#008B8B", "#236B8E", "#33A1DE", "#5D92B1", "#6E7B8B", "#33A1C9", "#4A708B", "#87CEFF", "#FFCC11", "#0D4F8B")) +
  theme_minimal() +
  theme(text=element_text(size=12, color="grey15"), panel.background = element_rect(fill = "white", colour = "grey80"), axis.title=element_text(size=12, color="grey15"), legend.text=element_text(size=10))+
  theme(legend.position = "none") +
  theme (panel.grid.minor.x =element_blank(), panel.grid.minor.y =element_blank())
